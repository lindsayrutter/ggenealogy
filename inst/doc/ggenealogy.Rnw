...
\title{Vignette: Grammar of graphics of genealogy (ggenealogy package) in R}
...
\setcounter{footnote}{1} \footnotetext{This \LaTeX\ vignette document
  ...
  \pkg{ggenealogy}. It is automatically dowloaded together with the
...
<<label=plotPathOnTree2, include=FALSE>>=
...
\begin{figure} 
  \begin{center} 
<<label=plotPathOnTree2,fig=TRUE,echo=FALSE>>= 
<<plotPathOnTree2>>
@ 
\end{center} 
