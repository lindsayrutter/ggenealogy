...
@
\end{center}
\caption{The shortest path between varieties ``Tokyo" and ``Narow" is strictly composed of a unidirectional sequence of parent-child relationships.}
\label{fig:plotPath2}
\end{figure}
...
Here, we did not change any defaults. As such, the resulting plot in Figure~\ref{fig:plotYearMatrix1} contains the default value of ``Variety" for the x-axis and y-axis labels, and the default value of ``Difference in years" for the legend key label. It also uses the default colors of dark blue for small year difference and light blue for large year difference.
