...
# start and end nodes indices in each edges
start=c(1, 1, 1, 5)
end=c(18, 19, 20, 6)

# fake data (scatterplot/network)
...
# without line
#g4 = ggplot(dat, aes(x = xvar, y = yvar)) + geom_point(shape = 20, size = 5, aes(color=cond))
# with line
#g4 = ggplot(dat, aes(x = xvar, y = yvar)) + geom_point(shape = 20, size = 5, aes(color=cond)) + geom_line(aes(x = xvar, y = yvar, group = gr), data = dat2)

# real data (scatterplot/network of statistics genealogy).
load(system.file("myDatSmall.RData", package="ggenealogy"))
dat$label = factor(dat$label)
dat$cond=factor(rep(c("A","B"), each=50))
dat2 = cbind(dat[c(start, end), ], gr = 1:length(start))
# without line
#g4 = ggplot(dat, aes(x=xvar, y=yvar)) + geom_point(shape=19, aes(color = cond), size=5)
# with line
g4 = ggplot(dat, aes(x=xvar, y=yvar)) + geom_point(shape=19, aes(color = cond), size=5) + geom_line(aes(x = xvar, y = yvar, group = gr), data = dat2)
